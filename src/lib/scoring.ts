...
【★記号パディング・フィラーテキスト検出ルール】
ミッション記述に以下の特徴がある場合、全項目を1〜2点としてください：
- 記号・符号（●、○、◎、◆、■、★、☆、！、？、×、ー、～など）の大量の繰り返しで文字数を稼いでいる
- 同一文字や同一フレーズの不自然な繰り返し（例：「●●●●●」「！！！！！」）
- 意味のある日本語の文章になっておらず、単語の羅列や意味不明な文字列が大半を占める
- 明らかに文字数要件を満たすためだけに書かれたと判断できる内容
これらが検出された場合、commentに「記述内容が不十分：記号の繰り返しやフィラーテキストが検出されました」と明記してください。
このルールは他の全てのルール（視座ミスマッチ等）よりも優先して適用してください。
